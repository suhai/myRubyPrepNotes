...
def base2to10(binary)
  result = [0]
  binary.reverse.chars.each_with_index do |val, idx|
    result << (val.to_i * 2 ** idx)
  end
  result.reduce(:+)
end
...
class Hash
  ...
  def my_select
    result = {}
    self.each { |key, val| result[key] = val if yield(key, val) }
    result
  end
end
...
class Hash
    ...
    self.each do |key, val|
      result[key] =  hash[key] ? prc.call(key, val, hash[key]) : val
    end

    hash.each do |key, val|
      result[key] = val if result[key].nil?
    end
...
def longest_palindrome(string)
  string.length.downto(3) do |num|
    string.chars.each_cons(num) { |subStr| return subStr.length if subStr == subStr.reverse }
  end
